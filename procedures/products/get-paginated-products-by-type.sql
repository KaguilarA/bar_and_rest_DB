...
DROP PROCEDURE IF EXISTS GetPaginatedProductsByType$$

-- Create a new procedure to get products by type
CREATE PROCEDURE GetPaginatedProductsByType(
    IN p_page INT, 
    IN p_page_size INT,
    IN p_type_id INT
)
BEGIN
    ...
    -- Select products with the specified type along with their type, state, and images information
    SELECT 
        p.id, 
        p.name, 
        p.description, 
